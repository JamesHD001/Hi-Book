...
export default async function MessagesPage() {
  ...
  const { data: conversationData } = ids.length
    ? await supabase
        .from("conversations")
  ...
  const otherUserIds = ((allParticipants ?? []) as Participant[])
  ...
  for (const row of (allParticipants ?? []) as Participant[]) {
    if (row.user_id !== user.id) otherByConversation.set(row.conversation_id, row.user_id);
  }
  ...
  const items = await Promise.all(
    conversations.map(async (conversation) => {
      ...
      const avatar = profile?.avatar_path
        ...
        : null;
      return { conversation, profile, avatar };
    }),
  );
    ...
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {!items.length ? (
          ...
          items.map(({ conversation, profile, avatar }) => {
            const name = profile?.display_name ?? "Conversation";
            return (
              <Link key={conversation.id} href={`/messages/${conversation.id}`} className="flex items-center gap-4 border-b border-slate-100 p-5 last:border-b-0 hover:bg-slate-50">
                {avatar ? (
                  <img src={avatar} alt="" className="h-12 w-12 shrink-0 rounded-full object-cover" />
                ) : (
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-slate-200 font-bold text-slate-600">{initials(name)}</div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="truncate font-semibold text-slate-950">{name}</p>
                  {profile?.username ? <p className="truncate text-sm text-slate-500">@{profile.username}</p> : <p className="text-sm text-slate-500">Direct conversation</p>}
                </div>
                <time dateTime={conversation.updated_at} className="shrink-0 text-xs text-slate-400">{formatDate(conversation.updated_at)}</time>
              </Link>
            );
          })
        )}
