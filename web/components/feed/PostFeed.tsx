...
export default function PostFeed() {
  ...
  const [scope, setScope] = useState<Scope>("HOME"); const [posts, setPosts] = useState<FeedPost[]>([]); const [cursor, setCursor] = useState<Cursor>(null); const [loading, setLoading] = useState(true); const [loadingMore, setLoadingMore] = useState(false); const [error, setError] = useState<string | null>(null); const [likes, setLikes] = useState<Record<string, LikeState>>({}); const [likeBusy, setLikeBusy] = useState<Record<string, boolean>>({});
  async function hydrateLikes(items: FeedPost[]) { const entries = await Promise.all(items.map(async (post) => { const { data } = await supabase.rpc("get_post_like_state", { target_post_id: post.post_id }); const state = data?.[0]; return state ? [post.post_id, { liked: Boolean(state.liked), like_count: Number(state.like_count) }] as const : null; })); setLikes((current) => ({ ...current, ...Object.fromEntries(entries.filter((entry): entry is readonly [string, LikeState] => entry !== null)) })); }
  async function load(nextScope: Scope, nextCursor: Cursor = null, append = false) { if (append) setLoadingMore(true); else setLoading(true); setError(null); try { const params = new URLSearchParams({ scope: nextScope }); if (nextCursor) { params.set("before_created_at", nextCursor.created_at); params.set("before_post_id", nextCursor.post_id); } const response = await fetch(`/api/feed?${params}`, { cache: "no-store" }); const payload = (await response.json()) as { posts?: FeedPost[]; next_cursor?: Cursor; error?: string }; if (!response.ok) throw new Error(payload.error ?? "Could not load the feed."); const nextPosts = payload.posts ?? []; setPosts((current) => append ? [...current, ...nextPosts] : nextPosts); setCursor(payload.next_cursor ?? null); void hydrateLikes(nextPosts); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not load the feed."); } finally { setLoading(false); setLoadingMore(false); } }
  async function toggleLike(postId: string) { if (likeBusy[postId]) return; setLikeBusy((current) => ({ ...current, [postId]: true })); try { const { data, error: rpcError } = await supabase.rpc("toggle_post_like", { target_post_id: postId }); if (rpcError) throw rpcError; const liked = Boolean(data); setLikes((current) => { const previous = current[postId] ?? { liked: false, like_count: 0 }; return { ...current, [postId]: { liked, like_count: Math.max(0, previous.like_count + (liked ? 1 : -1)) } }; }); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not update the like."); } finally { setLikeBusy((current) => ({ ...current, [postId]: false })); } }
  ...
  return <section aria-label="Post feed" className="space-y-4">
    ...
    {!loading && posts.map((post) => { const like = likes[post.post_id] ?? { liked: false, like_count: 0 }; return <article key={post.post_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
