...
select ok((select proconfig @> array['search_path='] from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'handle_auth_user_created'), 'registration trigger has empty controlled search_path');
...
select ok((select proconfig @> array['search_path='] from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'complete_registration'), 'complete_registration has empty controlled search_path');
