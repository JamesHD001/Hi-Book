...
select is((select has_function_privilege('public.get_moderation_queue()','EXECUTE','public')), false, 'moderation queue is not executable by PUBLIC');
select is((select has_function_privilege('public.get_moderation_queue()','EXECUTE','anon')), false, 'moderation queue is not executable by anon');
...
select is((select has_function_privilege('public.is_user_restricted(uuid)','EXECUTE','anon')), false, 'restriction helper is not executable by anon');
...
select is((select has_function_privilege('public.can_message_user(uuid)','EXECUTE','anon')), false, 'message authorization is not executable by anon');
