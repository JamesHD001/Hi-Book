...
select throws_ok($a$ update public.users set account_status='DEACTIVATED' where id='50000000-0000-0000-0000-000000000011' $a$,'P0001',null,'clients cannot directly change account status');
select throws_ok($b$ update public.users set deleted_at=now() where id='50000000-0000-0000-0000-000000000011' $b$,'P0001',null,'clients cannot directly set deletion timestamp');
