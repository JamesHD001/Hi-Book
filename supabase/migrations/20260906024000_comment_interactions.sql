...
begin
  ...
  if new.parent_comment_id is not null then
    select c.id, c.post_id, c.parent_comment_id, c.status into parent_record
    from public.comments c
    where c.id = new.parent_comment_id;

    if not found then raise exception 'Parent comment does not exist'; end if;
    if parent_record.post_id <> new.post_id then raise exception 'Parent comment must belong to the same post'; end if;
    if parent_record.parent_comment_id is not null then raise exception 'Replies cannot be nested more than one level'; end if;
    if parent_record.status <> 'PUBLISHED' then raise exception 'Cannot reply to an unavailable comment'; end if;
  end if;
...
begin
  ...
  if new.parent_comment_id is not null then
    select c.user_id into recipient from public.comments c where c.id = new.parent_comment_id;
  ...
  if exists (
    ...
    where (b.blocker_id = recipient and b.blocked_id = new.user_id)
       or (b.blocker_id = new.user_id and b.blocked_id = recipient)
  ) then return new; end if;

  insert into public.notifications (recipient_id, actor_id, type, entity_type, entity_id)
  select recipient, new.user_id, notification_kind, 'COMMENT'::public.notification_entity_type, new.id
  where exists (select 1 from public.users u where u.id = new.user_id and u.account_status = 'ACTIVE')
...
begin
  ...
  select c.post_id, c.user_id into target_post_id, target_comment_user_id
  from public.comments c
  where c.id = target_comment_id and c.status = 'PUBLISHED';

  if target_post_id is null or not public.can_view_post(target_post_id) then raise exception 'Comment is not available'; end if;

  if exists (
    select 1 from public.blocks b
    where (b.blocker_id = auth.uid() and b.blocked_id = target_comment_user_id)
       or (b.blocker_id = target_comment_user_id and b.blocked_id = auth.uid())
  ) then raise exception 'Comment interaction is unavailable'; end if;
  ...
  if existing_like is null then
    insert into public.comment_likes (comment_id, user_id) values (target_comment_id, auth.uid());
  else
    delete from public.comment_likes where id = existing_like;
  end if;

  return query
  select exists (select 1 from public.comment_likes cl where cl.comment_id = target_comment_id and cl.user_id = auth.uid()),
...
begin
  ...
  insert into public.notifications (recipient_id, actor_id, type, entity_type, entity_id)
  select recipient, new.user_id, 'COMMENT_LIKE'::public.notification_type, 'COMMENT'::public.notification_entity_type, new.comment_id
  where exists (select 1 from public.users u where u.id = new.user_id and u.account_status = 'ACTIVE')
    and exists (select 1 from public.users u where u.id = recipient and u.account_status = 'ACTIVE');
...
set search_path = ''
as $$
  select c.id, c.user_id, pr.username, pr.display_name, pr.avatar_path,
