...
create index if not exists product_fulfillment_rules_product_idx
  on public.product_fulfillment_rules(product_id, is_active);

-- A product has at most one active rule for a given fulfillment type.
create unique index if not exists product_fulfillment_rules_active_uq
  on public.product_fulfillment_rules(product_id, fulfillment_type)
...
create unique index if not exists fulfillments_purchase_uq
  on public.fulfillments(purchase_id);

create index if not exists fulfillments_user_idx
  on public.fulfillments(user_id, created_at desc);

create index if not exists fulfillments_status_idx
...
begin
  ...
  if not found then
    raise exception 'Fulfillment product does not exist';
  end if;

  if new.user_id <> v_purchase.user_id
     or new.product_id <> v_purchase.product_id then
...
create trigger trg_validate_fulfillment_row
before insert or update on public.fulfillments
for each row execute function public.validate_fulfillment_row();

-- ============================================================
-- 5. HBC FULFILLMENT RPC
-- ============================================================

create or replace function public.fulfill_hbc_purchase(
  p_purchase_id uuid,
...
begin
  if not public.is_trusted_server() then
    raise exception 'Purchase fulfillment is server-only';
  end if;

  if p_purchase_id is null or p_idempotency_key is null or btrim(p_idempotency_key) = '' then
    raise exception 'Purchase and idempotency key are required';
  end if;
  ...
  if v_purchase.status not in ('PAID','FULFILLED') then
    raise exception 'Purchase is not eligible for fulfillment';
  end if;
  ...
  select * into v_rule
   ...
   where product_id = v_purchase.product_id
     and fulfillment_type = 'COINS'
     and is_active = true
   for update;

  if not found then
    raise exception 'No active coin fulfillment rule exists for this product';
  end if;

  if v_rule.currency_id is null or v_rule.fulfillment_quantity is null then
    raise exception 'Coin fulfillment rule is incomplete';
  end if;

  select id into v_currency_id
    from public.currencies
   where id = v_rule.currency_id
     and code = 'HBC'
  ...
  if v_purchase.quantity > floor(9223372036854775807::numeric / v_rule.fulfillment_quantity) then
    raise exception 'Fulfillment quantity exceeds supported HBC range';
  end if;
  ...
  if found then
    ...
    if v_existing.fulfillment_rule_id <> v_rule.id
       or v_existing.delivered_quantity is distinct from v_hbc_amount then
      raise exception 'Existing fulfillment does not match current fulfillment rule';
    end if;
  else
  ...
  v_wallet_tx := public.credit_coin_wallet(
    v_purchase.user_id,
    v_currency_id,
    v_hbc_amount,
