...
-- A USER_RESTRICTED action is a dynamic authorization state. It expires
-- automatically when expires_at is reached; no background worker is required
...
revoke all on function public.is_user_restricted(uuid) from public;
grant execute on function public.is_user_restricted(uuid) to authenticated;

-- Execute a moderation action through one server-authoritative transaction.
...
begin
  ...
  insert into public.moderation_actions (
    ...
    target_starts_at,
    target_expires_at,
    target_metadata || jsonb_build_object('previous_account_status', case when v_previous_status is null then null else v_previous_status::text end)
...
-- Users can appeal an active moderation action belonging to themselves.
...
begin
  ...
  if appeal_reason is null or char_length(trim(appeal_reason)) < 10 or char_length(appeal_reason) > 5000 then
    raise exception 'Appeal reason must contain 10 to 5000 characters';
  end if;
  ...
  if not found then
    raise exception 'Moderation action is not appealable';
  end if;

  if v_action.target_type <> 'USER' or v_action.target_id <> auth.uid() then
    raise exception 'Appeal access denied';
  end if;

  insert into public.appeals (action_id, appellant_id, reason)
  values (target_action_id, auth.uid(), trim(appeal_reason))
  returning * into v_appeal;

  return v_appeal;
exception
  when unique_violation then
    raise exception 'An active appeal already exists for this action';
end;
$$;
...
-- Moderator appeal review. Reversing an account action restores the status that
-- existed immediately before the moderation action, rather than blindly forcing ACTIVE.
...
begin
  ...
  if decision in ('REVERSED','PARTIALLY_REVERSED') then
    ...
    if v_action.target_type = 'POST' and v_action.action_type in ('CONTENT_HIDDEN','CONTENT_REMOVED','CONTENT_RESTRICTED') then
