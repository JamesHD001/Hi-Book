-- Hi!Book 2.0 — atomic virtual gift transactions
-- Gifts consume sender HBC but do not transfer HBC to the recipient.
...
begin
  ...
  if public.is_blocked_between(p_sender_id, p_recipient_id) then
  ...
  -- The wallet RPC performs the row lock, balance check and atomic debit.
  v_debit_tx := public.debit_coin_wallet(
    p_sender_id,
    (select c.id from public.currencies c where c.code = 'HBC' and c.currency_type = 'VIRTUAL'),
    v_total,
  ...
  -- No recipient HBC credit occurs here. Future creator earnings are a
  -- separate economic entitlement with its own accounting rules.
  update public.coin_transactions
     set reference_id = v_tx.id
   where id = v_debit_tx
     and reference_type = 'GIFT_TRANSACTION';
...
create unique index if not exists gift_transactions_sender_idempotency_uq
  on public.gift_transactions(sender_id, idempotency_key)
  where idempotency_key is not null;

create index if not exists gift_transactions_recipient_created_idx
  on public.gift_transactions(recipient_id, created_at desc);

create index if not exists gift_transactions_status_created_idx
  on public.gift_transactions(status, created_at desc);
