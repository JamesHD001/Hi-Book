...
-- Efficient inbox retrieval: return at most one latest message per requested
-- conversation in one server-side query. The (created_at, id) ordering makes
-- the result deterministic even when messages share the same timestamp.
create index if not exists messages_conversation_created_id_idx
  on public.messages (conversation_id, created_at desc, id desc);

create or replace function public.get_latest_messages_for_conversations(
...
begin
  ...
  if coalesce(cardinality(target_conversation_ids), 0) > 500 then
    raise exception 'Too many conversations requested';
  end if;
  ...
  join public.conversations c
   ...
   and c.type = 'DIRECT'
  where m.conversation_id = any(coalesce(target_conversation_ids, array[]::uuid[]))
  order by m.conversation_id, m.created_at desc, m.id desc;
end;
$$;

revoke all on function public.get_latest_messages_for_conversations(uuid[]) from public, anon;
grant execute on function public.get_latest_messages_for_conversations(uuid[]) to authenticated;

-- Moderation actions require the dedicated action-execution permission. Case
-- assignment is a separate capability and must not implicitly authorize
-- enforcement.
...
begin
  ...
  if v_case.status in ('RESOLVED','CLOSED') then
    raise exception 'Moderation case is already closed';
  end if;
  ...
  if target_action_type in ('USER_RESTRICTED','USER_SUSPENDED','USER_DEACTIVATED','USER_BANNED') then
    if v_case.target_type <> 'USER' then
      raise exception 'User action requires a USER moderation case';
    end if;
    ...
    if v_case.target_type <> 'MESSAGE' then
      raise exception 'Message action requires a MESSAGE moderation case';
    end if;
...
-- Keep restriction helpers on a fixed empty search path as a defense-in-depth
-- measure for SECURITY DEFINER execution.
...
-- PARTIALLY_REVERSED is a recorded adjudication, not a full revocation.
-- A partial outcome must not silently restore an account/content item or
-- revoke the underlying moderation action. A reviewer can apply a separate,
-- explicitly authorized moderation action when the partial remedy requires a
...
begin
  ...
  insert into public.notifications (
    ...
    case
      ...
      when decision = 'PARTIALLY_REVERSED' then 'Your moderation appeal was partially accepted. The original action remains in effect unless separately changed.'
